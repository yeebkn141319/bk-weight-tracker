...
import os, json, io, base64, hashlib, secrets
...
def init_db():
    ...
    if PG:
        ...
        if not coach:
            conn.execute("INSERT INTO coach (username, password, name) VALUES (?, ?, ?)", ['bk', 'coach_only', 'BK'])
        conn.commit()
    conn.close()
...
def require_coach(): return 'coach_id' in session
def require_client(): return 'client_id' in session
...
def login():
    if request.method == 'POST':
        ...
        try:
            ...
            conn.close()
            if c:
                session['coach_id']=c['id']; session['coach_name']=c.get('name','BK'); session.permanent=True
...
def coach_dash():
    ...
    for c in rows:
        ...
        if d is None: s='new'; lbl='🆕 New'; clr='#8a8a9a'
        elif d <= 10: s='ok'; lbl=f'✅ {d}d'; clr='#4ec97e'
        elif d <= 14: s='soon'; lbl=f'⏰ Due in {14-d}d'; clr='#f0d07a'
        elif d <= 18: s='warn'; lbl=f'⚠️ {d-14}d overdue'; clr='#ff8c00'
...
def add_client():
    ...
    if request.method == 'POST':
        ...
        if not name or not phone: return render_template('add_client.html',error='Name & phone required')
        conn = get_db()
        try:
            _exec(conn, "INSERT INTO clients (name,phone,birthday,height,gender,password,goal_weight) VALUES (?,?,?,?,?,?,?)",
                  [name,phone,bday,height,gender,pw,goal_w])
...
def add_session(cid):
    ...
    _exec(conn, "INSERT INTO sessions (client_id,date,weight,body_fat,visceral_fat,muscle_mass,bmr,body_age,waist,abdomen,hip,thigh,notes) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
          [cid, d.get('date',date.today().isoformat()), _f(d.get('weight')), _f(d.get('body_fat')), _f(d.get('visceral_fat')),
           _f(d.get('muscle_mass')), _f(d.get('bmr')), _f(d.get('body_age')), _f(d.get('waist')), _f(d.get('abdomen')), _f(d.get('hip')), _f(d.get('thigh')), d.get('notes','')])
    conn.commit(); conn.close()
...
def upload_photo(cid):
    ...
    sd = request.form.get('date',date.today().isoformat())
    if file and allowed_file(file.filename):
        ext = file.filename.rsplit('.',1)[1].lower()
        fn = f"c{cid}_{datetime.now().strftime('%Y%m%d%H%M%S')}.{ext}"
        path = os.path.join(app.config['UPLOAD_FOLDER'], fn)
        file.save(path)
        try:
            img = Image.open(path); draw = ImageDraw.Draw(img)
            dt = f"📅 {sd}"
            try: font = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", max(20, img.width//25))
            except: font = ImageFont.load_default()
            bb = draw.textbbox((0,0), dt, font=font)
            tw, th = bb[2]-bb[0], bb[3]-bb[1]
            draw.rectangle([5, img.height-th-15, 5+tw+10, img.height-5], fill=(0,0,0,180))
            draw.text((7, img.height-th-13), dt, fill=(255,255,255), font=font)
            img.save(path)
        except: pass
        conn = get_db()
...
def client_view():
    ...
    photos = _exec(conn, "SELECT * FROM photos WHERE client_id=? ORDER BY date DESC", [cid]).fetchall()
    chart_rows = _exec(conn, "SELECT date,weight,body_fat,waist,hip,visceral_fat,muscle_mass,bmr,body_age,abdomen,thigh FROM sessions WHERE client_id=? ORDER BY date ASC", [cid]).fetchall()
    conn.close()
    return render_template('client.html',client=client,sessions=sessions,photos=photos,chart_json=json.dumps([dict(r) for r in chart_rows]),sessions_json=json.dumps([dict(r) for r in sessions]),photos_json=json.dumps([dict(r) for r in photos]))
...
@app.route('/_dbcheck')
def db_check():
    s = f'PG: {PG}<br>DB: {_DB_URL[:30]}...<br>'
    try:
        conn = get_db()
        cur = _exec(conn, "SELECT username FROM coach")
        s += f'Coaches: {[r["username"] for r in cur.fetchall()]}<br>OK'
        conn.close()
